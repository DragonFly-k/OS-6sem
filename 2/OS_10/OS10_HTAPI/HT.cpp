...
//		}
//
//		long size = sizeof(HTHANDLE) + (MaxKeyLength + MaxPayloadLength + sizeof(int) * 2) * Capacity;
//
...
//		DWORD SnapshotThread = NULL;
//
//		// Create unique mutex name based on the file name and current process id
//		std::wstring mutexName = L"Global\\" + std::wstring(FileName) + L"_" + std::to_wstring(GetCurrentProcessId());
//
...
//		hMutex = CreateMutex(NULL, 0, mutexName.c_str());
//		if (!hMutex)
//		{
...
//			CloseHandle(hMap);
//			CloseHandle(hFile);
//			throw std::runtime_error("Failed to create snapshot thread.");
//		}
//
//		ht->File = hFile;
//		ht->FileMapping = hMap;
...
//	{
//		ZeroMemory(ht->LastErrorMessage, 512);
//		memcpy(ht->LastErrorMessage, (char*)message, 512);
//	}
//
...
namespace HT
{
	...
	HTHANDLE* Create(int	  Capacity, int   SecSnapshotInterval, int   MaxKeyLength, int   MaxPayloadLength, const char FileName[512]) {
		...
		hFile = CreateFileA(FileName, GENERIC_READ | GENERIC_WRITE, NULL, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (!hFile)
		{
			throw "create file error";
		}
		...
		hMutex = CreateMutexA(NULL, 0, FileName);
		if (!hMutex)
		{
			throw "create mutex error";
		}
	...
	HTHANDLE* Open(const char FileName[512]) {
		...
		hFile = CreateFileA(FileName, GENERIC_READ | GENERIC_WRITE, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (!hFile)
		{
			throw "create file error";
		}
	...
	BOOL Insert(HTHANDLE* ht, Element* el) {
		if (ht->Capacity == ht->ElementCount)
		{
			SetLastError(ht, "Hash table is full");
			return FALSE;
	...
	BOOL Delete(HTHANDLE* ht, Element* el) {
		if (!CheckElementParm(ht, el))
			return FALSE;
		WaitForSingleObject(ht->Mutex, INFINITE);
		int indexInHT = -1;
	...
	Element* Get(HTHANDLE* ht, Element* el) {
		if (!CheckElementParm(ht, el))
			return NULL;
		WaitForSingleObject(ht->Mutex, INFINITE);
		int indexInHT = -1;
	...
	BOOL Update(HTHANDLE* ht, Element* oldelement, const void* newpayload, int  newpayloadlength) {
		if (!CheckElementParm(ht, oldelement) || !CheckElementParm(ht, newpayloadlength))
			return FALSE;
		WaitForSingleObject(ht->Mutex, INFINITE);
		int indexInHT = -1;

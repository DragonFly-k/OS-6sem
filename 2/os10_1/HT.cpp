...
#pragma region API_LAYER
	...
	// емкость хранилища // переодичность сохранения в сек.  // максимальный размер ключа // максимальный размер данных // имя файла 
	...
	HTHANDLE* Open(const wchar_t FileName[512]) {
		...
		if (hf == INVALID_HANDLE_VALUE)return NULL;
		HANDLE hm = CreateFileMapping(hf,NULL,PAGE_READWRITE,0, 0, NULL);
		if (!hm)return NULL;
		LPVOID lp = MapViewOfFile(hm,FILE_MAP_ALL_ACCESS,0, 0, 0);
		if (!lp)return NULL;
